...
namespace GraphX.Logic.Algorithms.LayoutAlgorithms
{
        ...
        where TGraph : IBidirectionalGraph<TVertex, TEdge>, IMutableVertexAndEdgeSet<TVertex, TEdge>
    {
        ...
        #endregion Variables needed for the layout
        ...
        protected void Report(int currentIteration)
        {
            ...
            #endregion Copy the calculated positions
        ...
        private double CalcEnergyIfExchanged(int p, int q)
        {
            ...
            for (int i = 0; i < _vertices.Length - 1; i++)
            {
                for (int j = i + 1; j < _vertices.Length; j++)
                {
                    ...
                    energy += kIj / 2 * ((dx * dx) + (dy * dy) + (lIj * lIj) - (2 * lIj * Math.Sqrt((dx * dx) + (dy * dy))));
                }
            }
            return energy;
        }
