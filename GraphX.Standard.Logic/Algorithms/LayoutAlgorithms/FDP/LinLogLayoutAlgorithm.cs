...
namespace GraphX.Logic.Algorithms.LayoutAlgorithms
{
        ...
        where TGraph : IBidirectionalGraph<TVertex, TEdge>, IMutableVertexAndEdgeSet<TVertex, TEdge>
    {
        ...
        #endregion Member variables - privates
        ...
        public override void Compute(CancellationToken cancellationToken)
        {
            ...
            for (int step = 1; step <= Parameters.iterationCount; step++)
            {
                ...
                #endregion hûlési függvény meghatározása
        ...
        {
            // Copy positions
            foreach (LinLogVertex v in _vertices)
            {
        ...
        {
            LinLogVertex v = _vertices[index];
            Vector gravitationVector = _baryCenter - v.Position;
            double dist = gravitationVector.Length;
            double tmp = Parameters.gravitationMultiplier * _repulsionMultiplier * Math.Max(v.RepulsionWeight, 1) * Math.Pow(dist, Parameters.attractionExponent - 2);
        ...
        private double AddAttractionDirection(int index, ref Vector dir)
        {
            double dir2 = 0.0;
            LinLogVertex v = _vertices[index];
            foreach (LinLogEdge e in v.Attractions)
            {
                //onhurkok elhagyasa
                if (e.Target == v)
                {
        ...
        private double AddRepulsionDirection(int index, QuadTree quadTree, ref Vector dir)
        {
            ...
            if (dist != 0.0)
            {
                double tmp = _repulsionMultiplier * v.RepulsionWeight * quadTree.Weight
		...
		*/
        ...
        private double GetGravitationEnergy(int index)
        {
            ...
            double dist = (v.Position - _baryCenter).Length;
        ...
        private void InitAlgorithm()
        {
            ...
            Dictionary<TVertex, LinLogVertex> vertexMap = new Dictionary<TVertex, LinLogVertex>();
            ...
            {
                int attrIndex = 0;
                foreach (TEdge e in VisitedGraph.InEdges(v.OriginalVertex))
                {
                ...
                foreach (TEdge e in VisitedGraph.OutEdges(v.OriginalVertex))
                {
                    double weight = e is WeightedEdge<TVertex> ? ((e as WeightedEdge<TVertex>).Weight) : 1;
                    v.Attractions[attrIndex] = new LinLogEdge
                    {
