...
namespace GraphX.Logic.Algorithms.EdgeRouting
{
        ...
        where TVertex : class, IGraphXVertex
    {
        ...
        public void BundleAllEdges(TGraph graph, CancellationToken cancellationToken)
        {
            ...
            foreach (TEdge e in graph.Edges)
            {
                ...
                if (!e.IsSelfLoop)
                {
                    ...
                    List<Point> list2 = _edgeGroupData[key].ControlPoints.ToList();
        ...
        public void BundleEdges(TGraph graph, IEnumerable<TEdge> edges)
        {
            ...
            {
                KeyPair key = new KeyPair(e.Source.ID, e.Target.ID);
                _movedEdgeGroupData.TryGetValue(key, out EdgeGroupData ed);
                if (ed != null)
                {
                    List<Point> list2 = ed.ControlPoints.ToList();
        ...
        private void AddEdgeData(TEdge e)
        {
            ...
            _edgeGroupData.TryGetValue(key, out EdgeGroupData ed);

            if (ed == null)
            {
                Point p1 = VertexPositions[e.Source];// e.Vertices[0].Location;
                Point p2 = VertexPositions[e.Target];//e.Vertices[1].Location;
                ed = new EdgeGroupData { V1 = p1, V2 = p2, ID = key };
                Point mid = VectorTools.MidPoint(p1, p2);
                ed.Middle = mid;
        ...
        private float VisibilityCoefficient(EdgeGroupData ed1, EdgeGroupData ed2)
        {
            ...
            Point pn1 = VectorTools.Plus(pn, p1);
            Point pn2 = VectorTools.Plus(pn, p2);
            ...
            Point im = VectorTools.MidPoint(i1, i2);
        ...
        private void FindCompatibleEdges(Dictionary<KeyPair, EdgeGroupData> edgeSet, CancellationToken cancellationToken)
        {
            foreach (KeyValuePair<KeyPair, EdgeGroupData> p1 in edgeSet)
            {
                ...
                foreach (KeyValuePair<KeyPair, EdgeGroupData> p2 in _edgeGroupData)
                {
        ...
        {
            if (subdivisionPointsNum < 1)
            {
                return;
            }

            foreach (EdgeGroupData ed in _edgeGroupData.Values)
            {
        ...
        {
            foreach (EdgeGroupData ed in _movedEdgeGroupData.Values)
            {
        ...
        private void DivideEdge(EdgeGroupData ed, int subdivisionPointsNum)
        {
            ...
            Point[] sPoints = new Point[subdivisionPointsNum];
        ...
        */
        ...
        private void CalculateNewControlPoints(object o)
        {
            ...
            for (int i = 0; i < _subdivisionPoints; i++)
            {
                Point p = ed.ControlPoints[i];
                Point p1, p2;
                p1 = i == 0 ? ed.V1 : ed.ControlPoints[i - 1];
                p2 = i == (_subdivisionPoints - 1) ? ed.V2 : ed.ControlPoints[i + 1];
                //SizeF sp = new SizeF(p);
                Point f = VectorTools.Multiply(VectorTools.Plus(VectorTools.Minus(p1, p), VectorTools.Minus(p2, p)), ed.K);
                ...
                foreach (GroupPairData epd in ed.CompatibleGroups.Values)
                {
                    ...
                    else
                    {
                        ...
                        if (_directed && _repulseOpposite)
                        {
                            j = _repulsionCoefficient;
                        }
                    }
                ...
                Point move = new Point(f.X + r.X, f.Y + r.Y);
        ...
        public void StraightenEdges(TGraph graph, float s)
        {
            foreach (TEdge e in graph.Edges)
            {
                ...
                Point[] controlPoints = e.RoutingPoints;//(PointF[])e.GetValue(ReservedMetadataKeys.PerEdgeIntermediateCurvePoints);
                Point[] newControlPoints = new Point[controlPoints.Length];
        ...
        private readonly Semaphore _sem = new Semaphore(0, int.MaxValue);
