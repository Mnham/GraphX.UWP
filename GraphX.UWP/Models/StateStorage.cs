...
namespace GraphX.Controls.Models
{
        ...
        where TGraph : class, IMutableBidirectionalGraph<TVertex, TEdge>
    {
        ...
        {
            foreach (GraphState<TVertex, TEdge, TGraph> a in _states.Values)
            {
                a.Dispose();
            }
            _states.Clear();
        ...
        public virtual void LoadState(string id)
        {
            ...
            foreach (KeyValuePair<TVertex, Measure.Point> item in _states[id].VertexPositions)
            {
            ...
            //setup visible edges
            foreach (TEdge item in _states[id].VisibleEdges)
            {
                EdgeControl edgectrl = _area.ControlFactory.CreateEdgeControl(_area.VertexList[item.Source], _area.VertexList[item.Target],
                                                       item);
                _area.InsertEdge(item, edgectrl);
                //edgectrl.UpdateEdge();
            }
            _area.UpdateLayout();
            foreach (EdgeControl item in _area.EdgesList.Values)
            {
                item.UpdateEdge();
            }
        }
        ...
        protected virtual GraphState<TVertex, TEdge, TGraph> GenerateGraphState(string id, string description = "")
        {
            ...
            Dictionary<TVertex, Measure.Point> vposlist = _area.VertexList.ToDictionary(item => item.Key, item => item.Value.GetPositionGraphX());
            List<TEdge> vedgelist = (from item in _area.EdgesList where item.Value.Visibility == Visibility.Visible select item.Key).ToList();
