...
namespace GraphX.Controls
{
    ...
    public abstract class VertexControlBase : Control, IGraphControl
    {
        ...
        public static readonly DependencyProperty VertexShapeProperty = DependencyProperty.Register(
            nameof(VertexShape),
            typeof(VertexShape),
            typeof(VertexControlBase),
        ...
        {
            if (VertexLabelControl is IAttachableControl<VertexControl> control)
            {
                control.Detach();
            }
        ...
        public IVertexConnectionPoint GetConnectionPointAt(Point position)
        {
            ...
            {
                Rect rect = new(a.RectangularSize.X, a.RectangularSize.Y, a.RectangularSize.Width, a.RectangularSize.Height);
        ...
        {
            IVertexConnectionPoint result = VertexConnectionPointsList.FirstOrDefault(a => a.Id == id);
            result?.Update();
            return result;
        }
        ...
        {
            foreach (IVertexConnectionPoint item in VertexConnectionPointsList)
            {
                if (isVisible)
                {
                    item.Show();
        ...
        {
            return round
                ? new Measure.Point(final ? (int)GraphAreaBase.GetFinalX(this) : (int)GraphAreaBase.GetX(this), final ? (int)GraphAreaBase.GetFinalY(this) : (int)GraphAreaBase.GetY(this))
                : new Measure.Point(final ? GraphAreaBase.GetFinalX(this) : GraphAreaBase.GetX(this), final ? GraphAreaBase.GetFinalY(this) : GraphAreaBase.GetY(this));
        }
        ...
        {
            VertexControlBase obj = d as VertexControlBase;
